...
struct mongoc_stream_inproc_t {
    mongoc_stream_t vtable;
    mongo::transport::TransportLayerInProc::Connection* wrapped;
};
...
void repairDatabasesAndCheckVersion(OperationContext* txn) {
    ...
    if (storageGlobalParams.repair) {
        ...
        for (vector<string>::const_iterator i = dbNames.begin(); i != dbNames.end(); ++i) {
            ...
            fassert(918506, repairDatabase(txn, storageEngine, dbName));
        }
    }
...
ExitCode _initAndListen(int listenPort) {
    ...
    transport::TransportLayerInProc::Options options;
    ...
    auto transportLayer = stdx::make_unique<transport::TransportLayerInProc>(options, sepPtr);
...

    {
        ...
        ss << "*********************************************************************" << endl;
        uassert(910296, ss.str().c_str(), boost::filesystem::exists(storageGlobalParams.dbpath));
    }

    {
        stringstream ss;
        ss << "repairpath (" << storageGlobalParams.repairpath << ") does not exist";
        uassert(912590, ss.str().c_str(), boost::filesystem::exists(storageGlobalParams.repairpath));
    }
...
transport::TransportLayerInProc::Connection* mongo_TransportLayerInProcConnectionConnect(const char* addr) {
    return transport::TransportLayerInProc::getTLByAddr(addr)->connectInProcClient();
}


ssize_t mongo_TransportLayerInProcConnectionPoll(mongoc_stream_poll_t* streams, size_t nstreams) {
    ssize_t nActive = 0;

    for (size_t i = 0; i < nstreams; ++i) {
        auto s = reinterpret_cast<mongoc_stream_inproc_t*>(streams[i].stream);

        streams[i].revents = 0;

        if (streams[i].events | POLLIN && s->wrapped->clientCanRead()) {
            streams[i].revents |= POLLIN;
            nActive++;
        }

        if (streams[i].events | POLLOUT && s->wrapped->clientCanWrite()) {
            streams[i].revents |= POLLOUT;
            nActive++;
        }
    }
...
ssize_t mongo_TransportLayerInProcConnectionWriteV(transport::TransportLayerInProc::Connection* connection,
                                                   mongoc_iovec_t* iov,
                                                   size_t iovcnt,
                                                   int32_t timeout_msec) {
    ssize_t out = 0;
    for (size_t i = 0; i < iovcnt; ++i) {
        if (! connection->clientSend(iov[i].iov_base, iov[i].iov_len, stdx::chrono::milliseconds(timeout_msec))) {
            return -1;
        }
...
ssize_t mongo_TransportLayerInProcConnectionReadV(transport::TransportLayerInProc::Connection* connection,
                                                  mongoc_iovec_t* iov,
                                                  size_t iovcnt,
                                                  int32_t timeout_msec) {
    ssize_t out = 0;
    for (size_t i = 0; i < iovcnt; ++i) {
        if (! connection->clientRecv(iov[i].iov_base, iov[i].iov_len, stdx::chrono::milliseconds(timeout_msec))) {
            return -1;
        }
...
void mongo_TransportLayerInProcConnectionClose(transport::TransportLayerInProc::Connection* connection) {
    connection->close();
}


static int _mongoc_stream_inproc_close(mongoc_stream_t* stream) {
    mongo_TransportLayerInProcConnectionClose(((mongoc_stream_inproc_t*)stream)->wrapped);

    return 0;
}


static void _mongoc_stream_inproc_destroy(mongoc_stream_t* stream) {
    /* nothing */
}


static ssize_t _mongoc_stream_inproc_poll(mongoc_stream_poll_t* streams,
                                          size_t nstreams,
                                          int32_t timeout) {
    return mongo_TransportLayerInProcConnectionPoll(streams, nstreams);
}


static ssize_t _mongoc_stream_inproc_readv(mongoc_stream_t* stream,
                                           mongoc_iovec_t* iov,
                                           size_t iovcnt,
                                           size_t min_bytes,
                                           int32_t timeout_msec) {
...
static ssize_t _mongoc_stream_inproc_writev(mongoc_stream_t* stream,
                                            mongoc_iovec_t* iov,
                                            size_t iovcnt,
                                            int32_t timeout_msec) {
...
mongoc_stream_t* inproc_stream_initiator(const mongoc_uri_t* uri,
                                         const mongoc_host_list_t* host,
                                         void* user_data,
                                         bson_error_t* error) {
    auto wrapped = mongo_TransportLayerInProcConnectionConnect(host->host_and_port);
    if (!wrapped) {
        return NULL;
    }

    mongoc_stream_inproc_t* s = (mongoc_stream_inproc_t*)bson_malloc0(sizeof *s);
...
int main(int argc, char** argv, char** envp) {
    mongoDbMain(argc, argv, envp);

    mongoc_init();

    auto uri = mongoc_uri_new("mongodb://completely_fake_not_real");

    constexpr size_t nWorkers = 5;
    constexpr size_t nOps = 5;

    std::array<stdx::thread, nWorkers> workers;

    for (size_t n = 0; n < workers.size(); n++) {
        const size_t local_n = n;
        workers[n] = stdx::thread([local_n, &uri]{
            auto client = mongoc_client_new_from_uri(uri);
            mongoc_client_set_stream_initiator(client, inproc_stream_initiator, nullptr);

            auto db = mongoc_client_get_database(client, "test");
            auto col = mongoc_database_get_collection(db, "test");

            for (size_t i = 0; i < nOps; ++i) {
                bson_t* doc = BCON_NEW("n", BCON_INT32(local_n), "i", BCON_INT32(i));

                mongoc_collection_insert(col, (mongoc_insert_flags_t)0, doc, nullptr, nullptr);
    ...
    mongoc_client_command_simple(client, "admin", cmd, nullptr, &out, nullptr);

    char* str = bson_as_json(&out, nullptr);
    log() << "ismaster: " << str << "\n";
    bson_free(str);
    bson_destroy(cmd);
    bson_destroy(&out);
    ...
    auto cursor = mongoc_collection_find_with_opts(col, &query, nullptr, nullptr);
...
//    auto tl = mongo::transport::TransportLayerInProc::getTLByAddr("foo"_sd);
//
//    auto conn = tl->connectInProcClient();
//
//    DataBuilder b;
...
//    b.writeAndAdvance<int32_t>(0);
//    b.writeAndAdvance<Terminated<'\0', StringData>>("admin.$cmd"_sd);
//    b.writeAndAdvance<int32_t>(0);
//    b.writeAndAdvance<int32_t>(1);
//    b.writeAndAdvance(BSON("ismaster" << 1));
//    b.getCursor().write<int32_t>(b.size());
//
//    conn->clientSend(b.getCursor().data(), b.size(), stdx::chrono::seconds(10));
//
//    char buf[10000];
//    conn->clientRecv(buf, 4, stdx::chrono::seconds(10));
//    conn->clientRecv(buf + 4, DataView(buf).read<int32_t>() - 4, stdx::chrono::seconds(10));
//

...
public:
    Recording(const StartRecordingTraffic& options)
        : _path(_getPath(options.getFilename().toString())),
          _maxLogSize(options.getMaxFileSize()),
          _toFlush(options.getBufferSize()) {

        _sss.setRunning(true);
        _sss.setBufferSize(options.getBufferSize());
        _sss.setRecordingFile(_path);
        _sss.setMaxFileSize(_maxLogSize);
    }
    ...
    void run() {
        _thread = stdx::thread([this] {
            try {
                ...
                std::fstream out(_path, out.binary | out.trunc | out.out);

                while (true) {
                    std::vector<Packet> storage;
                    _toFlush.popManyUpTo(1 << 24, std::back_inserter(storage));
     ...
     */
    bool pushRecord(const transport::SessionHandle& ts, Date_t now, const Message& message) {
        try {
            // If we pushed our packet, we're good
            if (_toFlush.tryPush(
                    {ts->id(), ts->local().toString(), ts->remote().toString(), now, message})) {
                return true;
            }

            // Otherwise we begin the process of failing the recording

            _toFlush.closeProducerEnd();
    ...
    BSONObj getStats() {
        stdx::lock_guard<stdx::mutex> lk(_mutex);
        _sss.setBufferedBytes(_toFlush.getStats().queueDepth);
        _sss.setCurrentFileSize(_written);
        return _sss.toBSON();
    }
...
private:
    ...
    MultiProducerSingleConsumerQueue<Packet, CostFunction> _toFlush;
    stdx::thread _thread;

    stdx::mutex _mutex;
    bool _inShutdown = false;
    TrafficRecorderSSS _sss;
    size_t _written = 0;
...
void TrafficRecorder::stop() {
    ...
    auto recording = [&] {
        ...
        auto recording = std::move(_recording);
        return recording;
    }();
...
void TrafficRecorder::observe(const transport::SessionHandle& ts,
    ...
    if (recording->pushRecord(ts, now, message)) {
        return;
    }
...
class TrafficRecorder::SSS : public ServerStatusSection {
public:
    SSS() : ServerStatusSection("trafficRecording") {}

...
namespace mongo {
    ...
    int CollectionCursorCache::eraseCursorGlobalIfAuthorized(int n, char* ids) {
        int numDeleted = 0;
        for ( int i = 0; i < n; i++ ) {
            long long ids_i;
            value_reader(ids_i).readFrom(ids + (i * sizeof(ids_i)));
            if ( eraseCursorGlobalIfAuthorized( ids_i ) )
                numDeleted++;
            if ( inShutdown() )
                break;
        }

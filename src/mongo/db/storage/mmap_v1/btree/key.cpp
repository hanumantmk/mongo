...
BSONObj KeyV1::toBson() const {
    ...
    while (1) {
        ...
        switch (bits & 0x3f) {
            ...
            case cdate:
                b.appendDate("", Date_t::fromMillisSinceEpoch(ConstDataView(reinterpret_cast<const char*>(p)).read<LittleEndian<long long>>()));
                p += 8;
                break;
            case cdouble:
                b.append("", ConstDataView(reinterpret_cast<const char*>(p)).read<LittleEndian<double>>());
                p += sizeof(double);
                break;
            case cint:
                b.append("", static_cast<int>(ConstDataView(reinterpret_cast<const char*>(p)).read<LittleEndian<double>>()));
                p += sizeof(double);
                break;
            case clong:
                b.append("", static_cast<long long>(ConstDataView(reinterpret_cast<const char*>(p)).read<LittleEndian<double>>()));
                p += sizeof(double);
                break;
            default:
                verify(false);
...
static int compare(const unsigned char*& l, const unsigned char*& r) {
    ...
    switch (lt) {
        ...
        case cdate: {
            long long L = ConstDataView(reinterpret_cast<const char*>(l)).read<LittleEndian<long long>>();
            long long R = ConstDataView(reinterpret_cast<const char*>(r)).read<LittleEndian<long long>>();
            if (L < R)
                return -1;
            if (L > R)
                return 1;
...
bool KeyV1::woEqual(const KeyV1& right) const {
    ...
    while (1) {
        ...
        switch (lval & cCANONTYPEMASK) {
            case coid:
                if (ConstDataView(reinterpret_cast<const char*>(l)).read<LittleEndian<uint32_t>>() !=
                    ConstDataView(reinterpret_cast<const char*>(r)).read<LittleEndian<uint32_t>>())
                    return false;
                l += 4;
                r += 4;
            case cdate:
                if (ConstDataView(reinterpret_cast<const char*>(l)).read<LittleEndian<unsigned long long>>() !=
                    ConstDataView(reinterpret_cast<const char*>(r)).read<LittleEndian<unsigned long long>>())
                    return false;
                l += 8;
                r += 8;
                break;

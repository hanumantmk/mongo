...
#define CEXPR_BSON_FROM_JSON(name, my_str)       \
    struct MAGIC_IMPL_##name {                   \
        ...
        CONSTEXPR static std::size_t str_len() { \
...
public:
    ...
    CONSTEXPR void append_slug(const uint8_t* from, size_t len) {
        memcpy(ptr, from, len);
        ptr += len;
...
template <typename T>
CONSTEXPR void append_num(
    T& b, const char* key, std::size_t klen, const char* v, std::size_t vlen) {
...
template <typename T>
CONSTEXPR void parse_impl(
    T& b, jsmn::jsmntok_t* toks, const char* v, int& i, int r, bool is_array) {

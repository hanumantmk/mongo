...
constexpr char MaxKeyInfo::className[];
constexpr JSFunctionSpec MaxKeyInfo::methods[];
...
void MaxKeyInfo::call(JSContext* cx, JS::CallArgs args) {
    ...
    ObjectWrapper o(cx, scope->maxKeyProto().proto());

    JS::RootedValue val(cx);

    if (!o.hasField("singleton")) {
        JS::RootedObject thisv(cx);
        scope->maxKeyProto().newObject(&thisv);
...
void MaxKeyInfo::postInstall(JSContext* cx, JS::HandleObject global, JS::HandleObject proto) {
    ...
    getScope(cx)->maxKeyProto().newObject(&value);

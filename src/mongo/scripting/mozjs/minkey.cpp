...
constexpr char MinKeyInfo::className[];
constexpr JSFunctionSpec MinKeyInfo::methods[];
...
void MinKeyInfo::call(JSContext* cx, JS::CallArgs args) {
    ...
    ObjectWrapper o(cx, scope->minKeyProto().proto());

    JS::RootedValue val(cx);

    if (!o.hasField("singleton")) {
        JS::RootedObject thisv(cx);
        scope->minKeyProto().newObject(&thisv);
...
void MinKeyInfo::postInstall(JSContext* cx, JS::HandleObject global, JS::HandleObject proto) {
    ...
    getScope(cx)->minKeyProto().newObject(&value);

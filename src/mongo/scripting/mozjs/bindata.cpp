...
constexpr JSFunctionSpec BinDataInfo::methods[];
constexpr JSFunctionSpec BinDataInfo::freeFunctions[];
constexpr char BinDataInfo::className[];
...
void hexToBinData(JSContext* cx, int type, StringData hexstr, JS::MutableHandleValue out) {
    ...
    return scope->binDataProto().newInstance(args, out);
}
...
void BinDataInfo::construct(JSContext* cx, JS::CallArgs args) {
    ...
    scope->binDataProto().newObject(&thisv);

...
constexpr JSFunctionSpec NativeFunctionInfo::methods[];
constexpr char NativeFunctionInfo::className[];
constexpr char NativeFunctionInfo::inheritFrom[];

void NativeFunctionInfo::construct(JSContext* cx, JS::CallArgs args) {
    auto scope = getScope(cx);

    scope->nativeFunctionProto().newObject(args.rval());
}
...
void NativeFunctionInfo::make(JSContext* cx,
    ...
    scope->nativeFunctionProto().newInstance(obj);

...
#include "mongo/executor/this_thread_task_executor.h"
...
class ThisThreadTaskExecutor::CallbackState : public TaskExecutor::CallbackState {
...
class ThisThreadTaskExecutor::EventState : public TaskExecutor::EventState {
...
ThisThreadTaskExecutor::ThisThreadTaskExecutor(std::unique_ptr<NetworkInterface> net)
    : _net(std::move(net)) {}

ThisThreadTaskExecutor::~ThisThreadTaskExecutor() {}
...
void ThisThreadTaskExecutor::shutdown() {
...
std::string ThisThreadTaskExecutor::getDiagnosticString() {
    return {};
}

Date_t ThisThreadTaskExecutor::now() {
    return _net->now();
}

StatusWith<TaskExecutor::EventHandle> ThisThreadTaskExecutor::makeEvent() {
...
void ThisThreadTaskExecutor::signalEvent(const EventHandle& event) {
    stdx::unique_lock<stdx::mutex> lk(_mutex);
    signalEvent_inlock(event, std::move(lk));
}

StatusWith<TaskExecutor::CallbackHandle> ThisThreadTaskExecutor::onEvent(const EventHandle& event,
...
void ThisThreadTaskExecutor::waitForEvent(const EventHandle& event) {
...
StatusWith<TaskExecutor::CallbackHandle> ThisThreadTaskExecutor::scheduleWork(
    const CallbackFn& work) {
    auto wq = makeSingletonWorkQueue(work);
    WorkQueue temp;
    stdx::unique_lock<stdx::mutex> lk(_mutex);
...
StatusWith<TaskExecutor::CallbackHandle> ThisThreadTaskExecutor::scheduleWorkAt(
...
StatusWith<TaskExecutor::CallbackHandle> ThisThreadTaskExecutor::scheduleRemoteCommand(
...
void ThisThreadTaskExecutor::cancel(const CallbackHandle& cbHandle) {
...
void ThisThreadTaskExecutor::wait(const CallbackHandle& cbHandle) {
    invariant(cbHandle.isValid());
    auto cbState = checked_cast<CallbackState*>(getCallbackFromHandle(cbHandle));
...
void ThisThreadTaskExecutor::appendConnectionStats(BSONObjBuilder* b) {
    _net->appendConnectionStats(b);
}

void ThisThreadTaskExecutor::cancelAllCommands() {
    _net->cancelAllCommands();
}

StatusWith<TaskExecutor::CallbackHandle> ThisThreadTaskExecutor::enqueueCallbackState_inlock(
...
ThisThreadTaskExecutor::WorkQueue ThisThreadTaskExecutor::makeSingletonWorkQueue(CallbackFn work,
                                                                                 Date_t when) {
    WorkQueue result;
...
ThisThreadTaskExecutor::EventList ThisThreadTaskExecutor::makeSingletonEventList() {
    EventList result;
...
void ThisThreadTaskExecutor::signalEvent_inlock(const EventHandle& event,
...
void ThisThreadTaskExecutor::scheduleIntoPool_inlock(WorkQueue* fromQueue,
                                                     stdx::unique_lock<stdx::mutex> lk) {
...
void ThisThreadTaskExecutor::drainPool(stdx::unique_lock<stdx::mutex> lk) {
    ...
    auto guard = MakeGuard([&] { _runningCallbacks = false; });

    WorkQueue active;
    while (_poolInProgressQueue.size()) {
        active.splice(active.end(), _poolInProgressQueue);
        lk.unlock();
        for (const auto& cbState : active) {
            runCallback(cbState);
        }
...
void ThisThreadTaskExecutor::runCallback(const std::shared_ptr<CallbackState>& cbStateArg) {

...
#include "mongo/executor/this_thread_task_executor_test_fixture.h"
...
MONGO_INITIALIZER(ThisThreadExecutorCommonTests)(InitializerContext*) {
    addTestsForExecutor("ThisThreadExecutorCommon",
                        [](std::unique_ptr<NetworkInterfaceMock>* net) {
                            return makeThisThreadTestExecutor(std::move(*net));
                        });
    return Status::OK();
}
...
TEST_F(ThisThreadExecutorTest, TimelyCancelationOfScheduleWorkAt) {
    auto net = getNet();

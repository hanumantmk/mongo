 ...
 */
class ThisThreadTaskExecutor final : public TaskExecutor {
    MONGO_DISALLOW_COPYING(ThisThreadTaskExecutor);

public:
    /**
     * Constructs an instance of ThisThreadTaskExecutor that runs tasks in "pool" and uses "net"
     * for network operations.
     */
    ThisThreadTaskExecutor(std::unique_ptr<NetworkInterface> net);

    /**
     * Destroys a ThisThreadTaskExecutor.
     */
    ~ThisThreadTaskExecutor();
...
private:
     ...
     */
    void runCallback(const std::shared_ptr<CallbackState>& cbState);

    void drainPool(stdx::unique_lock<stdx::mutex> lk);

...
#define CONN2ID(status, conn)                                                \
    [](const Status& status, const ConnectionPool::ConnectionHandle& conn) { \
        ASSERT(status.isOK());                                               \
        return static_cast<ConnectionImpl*>(conn.get())->id();               \
    }(status, conn)
...
TEST_F(ConnectionPoolTest, SameConn) {
    ...
    pool.get(HostAndPort(),
             Milliseconds(5000),
             [&](Status status, ConnectionPool::ConnectionHandle conn) {
                 conn1Id = CONN2ID(status, conn);
             });
...
TEST_F(ConnectionPoolTest, TimeoutOnSetup) {
    ...
    pool.get(HostAndPort(),
             Milliseconds(5000),
             [&](Status status, ConnectionPool::ConnectionHandle conn) { notOk = !status.isOK(); });
...
TEST_F(ConnectionPoolTest, refreshHappens) {
    ...
    pool.get(HostAndPort(),
             Milliseconds(5000),
             [&](Status status, ConnectionPool::ConnectionHandle conn) { ASSERT(status.isOK()); });
...
TEST_F(ConnectionPoolTest, refreshTimeoutHappens) {
    ...
    pool.get(HostAndPort(),
             Milliseconds(1000),
             [&](Status status, ConnectionPool::ConnectionHandle conn) {
                 ASSERT_NE(CONN2ID(status, conn), conn1Id);
                 reachedB = true;
             });
...
TEST_F(ConnectionPoolTest, hostTimeoutHappens) {
    ...
    pool.get(HostAndPort(),
             Milliseconds(5000),
             [&](Status status, ConnectionPool::ConnectionHandle conn) {
                 ASSERT_NE(connId, CONN2ID(status, conn));
                 reachedB = true;
             });
...
TEST_F(ConnectionPoolTest, hostTimeoutHappensMoreGetsDelay) {
    ...
    pool.get(HostAndPort(),
             Milliseconds(5000),
             [&](Status status, ConnectionPool::ConnectionHandle conn) {
                 ASSERT_EQ(connId, CONN2ID(status, conn));
                 reachedB = true;
             });
    ASSERT(reachedB);
...
TEST_F(ConnectionPoolTest, hostTimeoutHappensCheckoutDelays) {
    ...
    pool.get(HostAndPort(),
             Milliseconds(5000),
             [&](Status status, ConnectionPool::ConnectionHandle conn) {
                 ASSERT_EQ(conn2Id, CONN2ID(status, conn));
                 reachedA = true;
             });
    ...
    pool.get(HostAndPort(),
             Milliseconds(5000),
             [&](Status status, ConnectionPool::ConnectionHandle conn) {
                 ASSERT_NE(conn1Id, CONN2ID(status, conn));
                 ASSERT_NE(conn2Id, CONN2ID(status, conn));
                 reachedB = true;
             });
    ASSERT(reachedB);
}
...
TEST_F(ConnectionPoolTest, dropConnections) {
    ...
    pool.get(HostAndPort(),
             Milliseconds(5000),
             [&](Status status, ConnectionPool::ConnectionHandle conn) {
                 ASSERT_EQ(CONN2ID(status, conn), conn1Id);
    ...
    pool.get(HostAndPort(),
             Milliseconds(5000),
             [&](Status status, ConnectionPool::ConnectionHandle conn) {
                 ASSERT_NE(CONN2ID(status, conn), conn2Id);
                 reachedB = true;
             });

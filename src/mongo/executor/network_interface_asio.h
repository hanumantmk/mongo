...
// Explicit move construction and assignment to support MSVC
    ...
    /**
     * The explicit strand that all non-op operations. This must be the last
     * member of NetworkInterfaceASIO because any pending operation for the
     * strand are run when it's dtor is called. Any members that fall after it
     * will have already been destroyed, which will make those fields illegal
     * to touch from callbacks.
     */
    asio::io_service::strand _strand;
};

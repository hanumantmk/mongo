...
#define mmm(x)
    ...
    public:
        ...
        void append( Message& m ) {
            ...
            memcpy( _cur , view2ptr(m.singleData()) , m.header().len() );
            _cur += m.header().len();
        }
...
again:
            ...
            ScopeGuard guard = MakeGuard(free, view2ptr(md));
            verify(view2ptr(md));

            memcpy(view2ptr(md), &header, headerLen);
            int left = len - headerLen;

            psock->recv( md._data(), left );

            guard.Dismiss();
            m.setData(view2ptr(md), true);
            return true;
